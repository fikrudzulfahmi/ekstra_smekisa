...
const terapkan = () => {
    if (!filter.kelas_id) {
        showWarningAlert('Pilih kelas terlebih dahulu.');
        return;
    }
    router.get(route('admin.laporan.nilai.per-kelas'), { ...filter }, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const exportExcel = () => {
    ...
    window.open(
        route('admin.laporan.nilai.per-kelas.export', { ...filter }),
        '_blank'
    );
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between gap-3">
                ...
                <Link :href="route('admin.laporan.nilai')" class="text-sm font-medium text-[#3E6FD9] hover:underline print:hidden">
