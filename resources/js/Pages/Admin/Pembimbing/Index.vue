...
const submit = () => {
    if (isEditing.value) {
        form.put(route('admin.pembimbing.update', form.id), { onSuccess: resetForm });
    } else {
        form.post(route('admin.pembimbing.store'), { onSuccess: resetForm });
    }
};
...
const hapus = (item) => {
    showConfirm(`Hapus pembina ${item.nama}? Akun login-nya juga akan dihapus.`).then((result) => {
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
                ...
                <form @submit.prevent="submit" class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        <label class="mb-2 block text-sm font-medium text-[#1B2333]">Ekstra yang Dibimbing</label>
            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-black/5">
                <div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-100">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="item in pembimbing" :key="item.id" class="transition hover:bg-[#F4F7FC]/60">
