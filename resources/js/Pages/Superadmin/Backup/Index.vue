...
import { ref } from 'vue';
import { showConfirm, showWarningAlert } from '@/utils/sweetalert';
...
const buatBackup = () => {
    formBackup.post(route('superadmin.backup.create'), { preserveScroll: true });
};

const unduh = (path) => {
    window.location.href = route('superadmin.backup.download') + '?path=' + encodeURIComponent(path);
};

const hapus = (path) => {
    showConfirm('Hapus file backup ini?').then((result) => {
        if (result.isConfirmed) {
            router.delete(route('superadmin.backup.destroy'), { data: { path }, preserveScroll: true });
        }
    });
};

const restoreFile = ref(null);
const formRestore = useForm({ sql_file: null });
...
const doRestore = () => {
    ...
    showConfirm('PERHATIAN! Proses restore akan MENIMPA seluruh database saat ini. Lanjutkan?', 'Peringatan Restore').then((result) => {
        if (result.isConfirmed) {
            formRestore.post(route('superadmin.backup.restore'), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5 flex flex-col items-center text-center justify-center space-y-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-[#5B6472]">Buat cadangan data aplikasi saat ini dalam format file SQL.</p>
                    </div>
                ...
                <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5 flex flex-col items-center text-center justify-center space-y-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-[#5B6472]">Pulihkan database dari file <code class="rounded bg-gray-100 px-1 text-xs">.sql</code> backup.</p>
                    </div>
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-black/5 overflow-hidden">
                <div class="border-b border-gray-100 p-5 bg-[#F4F7FC] flex items-center justify-between">
                    <h3 class="font-['Poppins'] text-lg font-semibold text-[#0B1B36]">Riwayat Backup</h3>
                    <span class="text-xs text-[#5B6472]">{{ backups.length }} file tersimpan</span>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="item in backups" :key="item.path" class="transition hover:bg-[#F4F7FC]/60">
                                ...
                                <td class="px-5 py-3 text-right">
                                    <button @click="unduh(item.path)" class="rounded-lg px-3 py-1.5 text-sm font-medium text-[#3E6FD9] transition hover:bg-[#3E6FD9]/10 mr-1">Unduh</button>
